...
export default function LoanManagerMock() {
  ...
  async function loadStateFromServer() {
    hydrated.current = false;
    try {
      const res = await apiRequest('/api/legacy-loans/state');
  ...
  async function persistStateToServer(nextState) {
    try {
      await apiRequest('/api/legacy-loans/state', { method: 'PUT', body: nextState });
    } catch (err) {
  ...
  return (
    <div className="legacy-loan-manager min-h-screen bg-gray-50 text-gray-600 w-full">
      {/* Header */}
