...
function ensureAccountExists(id, label) {
  const account = getAccountById(db, id);
  assert(account, `${label} was not found.`);
  return account;
}

function ensureCashAccountExists(id, label) {
  const account = ensureAccountExists(id, label);
...
function normalizeTransactionPayload(body, existing = null) {
  ...
  ensureCashAccountExists(accountId, 'Account');
  ...
  if (direction === 'transfer') {
    ...
    ensureCashAccountExists(toAccountId, 'Destination account');
  ...
  if (recurringRuleId) {
    assert(getRecurringRuleById(db, recurringRuleId), 'Recurring rule was not found.');
  }
  return {
...
function normalizeRecurringRulePayload(body, existing = null) {
  const direction = normalizeDirection(body?.direction ?? existing?.direction);
  const accountId = normalizeInteger(body?.accountId ?? existing?.accountId, 'Source account', { min: 1 });
  ensureCashAccountExists(accountId, 'Source account');
  let toAccountId = normalizeInteger(body?.toAccountId ?? existing?.toAccountId, 'Destination account', { min: 1, nullable: true });
  if (direction === 'transfer') {
...
function normalizeRecurringOverridePayload(body, existing = null) {
  const ruleId = normalizeInteger(body?.ruleId ?? existing?.ruleId, 'Recurring rule', { min: 1 });
  const rule = getRecurringRuleById(db, ruleId);
  ...
  if (action === 'modify') {
    ...
    ensureCashAccountExists(accountId, 'Override account');
    if (rule.direction === 'transfer') {
      ...
      ensureCashAccountExists(toAccountId, 'Override destination account');
...
function normalizeReconciliationPayload(body, existing = null) {
  const accountId = normalizeInteger(body?.accountId ?? existing?.accountId, 'Reconciliation account', { min: 1 });
  ensureCashAccountExists(accountId, 'Reconciliation account');
...
function normalizeLoanDrawPayload(body, existing = null) {
  const accountId = normalizeInteger(body?.accountId ?? existing?.accountId, 'Loan account', { min: 1 });
  ensureLoanAccountExists(accountId, 'Loan account');
  return {
    accountId,
...
function buildBootstrapPayload(user) {
  ...
  return {
    ...
    recurringRules: listRecurringRules(db),
    recurringOverrides: listRecurringOverrides(db),
    budgets: listBudgets(db),
    reconciliations: listReconciliations(db),
    loanPayments: listLoanPayments(db),
    loanDraws: listLoanDraws(db),
  };
}
...
app.get('/api/legacy-loans/state', requireAuth, (req, res) => {
  res.json({ state: getLegacyLoanState(db, legacyLoanDefaultState) });
});

app.put('/api/legacy-loans/state', requireAuth, (req, res) => {
  try {
    const state = normalizeLegacyLoanState(req.body || {});
    res.json({ state: saveLegacyLoanState(db, state) });
  } catch (error) {
    sendError(res, error);
  }
});
...
app.post('/api/accounts', requireAuth, (req, res) => {
  try {
    ...
    res.status(201).json({ account: createAccount(db, account) });
  } catch (error) {
    sendError(res, error);
  }
});
...
app.delete('/api/accounts/:id', requireAuth, (req, res) => {
  try {
    ...
    const usage = getAccountUsage(db, id);
...
app.get('/api/transactions', requireAuth, (req, res) => {
  res.json({ transactions: listTransactions(db) });
});

app.post('/api/transactions', requireAuth, (req, res) => {
  try {
    const transaction = normalizeTransactionPayload(req.body);
    res.status(201).json({ transaction: createTransaction(db, transaction) });
  } catch (error) {
    sendError(res, error);
  }
});
...
app.post('/api/transactions/bulk', requireAuth, (req, res) => {
  try {
    ...
    const transactions = req.body.transactions.map((item) => normalizeTransactionPayload(item));
    const created = transactions.map((transaction) => createTransaction(db, transaction));
...
app.put('/api/transactions/:id', requireAuth, (req, res) => {
  try {
    ...
    const transaction = normalizeTransactionPayload(req.body, existing);
    res.json({ transaction: updateTransaction(db, id, transaction) });
  } catch (error) {
    sendError(res, error);
  }
});
...
app.get('/api/recurring-rules', requireAuth, (req, res) => {
  res.json({
    recurringRules: listRecurringRules(db),
    recurringOverrides: listRecurringOverrides(db),
  });
...
app.post('/api/recurring-rules', requireAuth, (req, res) => {
  try {
    const rule = normalizeRecurringRulePayload(req.body);
    res.status(201).json({ recurringRule: createRecurringRule(db, rule) });
  } catch (error) {
    sendError(res, error);
  }
});

app.put('/api/recurring-rules/:id', requireAuth, (req, res) => {
  try {
    const id = normalizeInteger(req.params.id, 'Recurring rule id', { min: 1 });
    const existing = getRecurringRuleById(db, id);
    if (!existing) {
      return res.status(404).json({ error: 'Recurring rule not found.' });
    }
    const rule = normalizeRecurringRulePayload(req.body, existing);
    res.json({ recurringRule: updateRecurringRule(db, id, rule) });
  } catch (error) {
    sendError(res, error);
  }
});

app.delete('/api/recurring-rules/:id', requireAuth, (req, res) => {
  try {
    const id = normalizeInteger(req.params.id, 'Recurring rule id', { min: 1 });
    const existing = getRecurringRuleById(db, id);
    if (!existing) {
...
app.post('/api/recurring-overrides', requireAuth, (req, res) => {
  try {
    const override = normalizeRecurringOverridePayload(req.body);
    res.status(201).json({ recurringOverride: upsertRecurringOverride(db, override) });
  } catch (error) {
    sendError(res, error);
  }
});

app.delete('/api/recurring-overrides/:id', requireAuth, (req, res) => {
  try {
    const id = normalizeInteger(req.params.id, 'Recurring override id', { min: 1 });
    const existing = getRecurringOverrideById(db, id);
    if (!existing) {
...
app.get('/api/budgets', requireAuth, (req, res) => {
  res.json({ budgets: listBudgets(db) });
});

app.post('/api/budgets', requireAuth, (req, res) => {
  try {
    const budget = normalizeBudgetPayload(req.body);
    res.status(201).json({ budget: createBudget(db, budget) });
  } catch (error) {
    sendError(res, error);
  }
});

app.post('/api/budgets/bulk', requireAuth, (req, res) => {
  try {
    ...
    req.body.budgets.forEach((item) => {
      ...
      if (existing) {
        updateBudget(db, existing.id, payload);
        summary.updated += 1;
      } else {
        ...
        if (duplicate) {
          updateBudget(db, duplicate.id, payload);
          summary.updated += 1;
        } else {
          createBudget(db, payload);
          summary.created += 1;
        }
      }
    });

    res.json({ summary, budgets: listBudgets(db) });
  } catch (error) {
    sendError(res, error);
  }
});
...
app.put('/api/budgets/:id', requireAuth, (req, res) => {
  try {
    ...
    res.json({ budget: updateBudget(db, id, budget) });
  } catch (error) {
    sendError(res, error);
  }
});
...
app.get('/api/reconciliations', requireAuth, (req, res) => {
  res.json({ reconciliations: listReconciliations(db) });
});

app.post('/api/reconciliations', requireAuth, (req, res) => {
  try {
    const reconciliation = normalizeReconciliationPayload(req.body);
    ...
    const saved = createReconciliation(db, reconciliation);
    ...
    res.status(201).json({ reconciliation: getReconciliationById(db, saved.id) });
  } catch (error) {
    sendError(res, error);
  }
});

app.delete('/api/reconciliations/:id', requireAuth, (req, res) => {
  try {
    const id = normalizeInteger(req.params.id, 'Reconciliation id', { min: 1 });
    const existing = getReconciliationById(db, id);
    if (!existing) {
      return res.status(404).json({ error: 'Reconciliation not found.' });
    }
    existing.transactionIds.forEach((transactionId) => {
      const transaction = getTransactionById(db, transactionId);
...
app.get('/api/loan-payments', requireAuth, (req, res) => {
  res.json({ loanPayments: listLoanPayments(db) });
});

app.post('/api/loan-payments', requireAuth, (req, res) => {
  try {
    const payment = normalizeLoanPaymentPayload(req.body);
    res.status(201).json({ loanPayment: createLoanPayment(db, payment) });
  } catch (error) {
    sendError(res, error);
  }
});

app.put('/api/loan-payments/:id', requireAuth, (req, res) => {
  try {
    const id = normalizeInteger(req.params.id, 'Loan payment id', { min: 1 });
    const existing = getLoanPaymentById(db, id);
    if (!existing) {
      return res.status(404).json({ error: 'Loan payment not found.' });
    }
    const payment = normalizeLoanPaymentPayload(req.body, existing);
    res.json({ loanPayment: updateLoanPayment(db, id, payment) });
  } catch (error) {
    sendError(res, error);
  }
});

app.delete('/api/loan-payments/:id', requireAuth, (req, res) => {
  try {
    const id = normalizeInteger(req.params.id, 'Loan payment id', { min: 1 });
    const existing = getLoanPaymentById(db, id);
    if (!existing) {
...
app.get('/api/loan-draws', requireAuth, (req, res) => {
  res.json({ loanDraws: listLoanDraws(db) });
});

app.post('/api/loan-draws', requireAuth, (req, res) => {
  try {
    const draw = normalizeLoanDrawPayload(req.body);
    res.status(201).json({ loanDraw: createLoanDraw(db, draw) });
  } catch (error) {
    sendError(res, error);
  }
});

app.delete('/api/loan-draws/:id', requireAuth, (req, res) => {
  try {
    const id = normalizeInteger(req.params.id, 'Loan draw id', { min: 1 });
    const existing = getLoanDrawById(db, id);
    if (!existing) {
